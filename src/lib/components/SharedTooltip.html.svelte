<!--
 ...
 <script>
    ...
    const { data, width, yScale, config, xGet, zGet, zRange } = getContext('LayerCake');
  ...
  >
    ...
    {#if visible === true}
      ...
      <div
        ...
        >
          <div class="title">Day {formatTitle(found[$config.x])}</div>
          {#each foundSorted as row, i}
          <!-- {console.log($zRange)} -->
            <div class="row" style="
            color:{$zRange[i] || 'black'}"><span class="key" >{formatKey(row.key)}:</span> {formatValue(row.value)}</div>
          {/each}
      </div>
    {/if}
  </QuadTree>
